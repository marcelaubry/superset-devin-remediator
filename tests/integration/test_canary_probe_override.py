"""Canary-only probe override: `LIVE_CANARY_ALLOW_MISSING_PROBE`.

Every scenario runs the real Phase 3 approval (Slack click + signed label webhook) and then
drives the worker with the fake Devin, GitHub, Slack and probe adapters, with `PROBE_ROOT`
pointed at an empty directory so the registry reports "no approved probe registered" exactly
as the live case does. The override is enabled *only* here.
"""
...
def _with_override(settings: Settings, **overrides: Any) -> Settings:
    return settings.model_copy(
        update={
            "live_canary": True,
            "live_canary_allow_missing_probe": True,
...
async def _overrides(h: RemediationHarness, case_id: Any) -> list[CanaryProbeOverride]:
    async with h.base.factory() as session:
        rows = await session.scalars(
            select(CanaryProbeOverride)
            .where(CanaryProbeOverride.case_id == case_id)
...
@pytest.mark.asyncio
async def test_missing_probe_still_blocks_without_the_override(
    rem: RemediationHarness,  # noqa: F811
    blocked: Case,
) -> None:
    assert await rem.attempts(blocked.id) == []
    assert await _overrides(rem, blocked.id) == []
    assert await _probe_rows(rem, blocked.id) == (0, 0)
    # Re-processing keeps it blocked: nothing resumes while the flag is false.
    case = await rem.step(blocked.id)
    assert case.state == CaseState.REMEDIATION_HUMAN_BLOCKED


@pytest.mark.asyncio
async def test_registry_errors_other_than_a_missing_probe_never_use_the_override(
    rem: RemediationHarness,  # noqa: F811
    tmp_path: Any,
) -> None:
    import shutil
    ...
    rem.base.settings = _with_override(rem.settings, probe_root=str(tmp_path / "probes"))
    ...
    assert await _overrides(rem, case.id) == []
...
async def test_blocked_case_resumes_to_ci_passed_without_any_probe_evidence(
    ...
    rem.base.settings = _with_override(_no_probes(rem.settings, tmp_path))
    ...
    assert attempt.canary_probe_override is True
    ...
    # No probe evidence of any kind was invented.
    assert await _probe_rows(rem, case.id) == (0, 0)

    # One append-only audit row with the full provenance of what was skipped.
    rows = await _overrides(rem, case.id)
    assert len(rows) == 1
    audit = rows[0]
    assert audit.repository == "apache/superset" and audit.issue_number == SUCCESS
    assert audit.triage_result_hash == requests[0].triage_result_hash
    assert audit.base_sha == attempt.base_sha
    assert audit.reason == CANARY_OVERRIDE_REASON
    assert audit.warning == CANARY_OVERRIDE_WARNING
    assert audit.approved_by == requests[0].decided_by_slack_user_id
    assert audit.actor == "config:LIVE_CANARY_ALLOW_MISSING_PROBE"

    # The two probe gates were skipped; PR and exact-head CI were not.
        ...
        reasons = list(
            (
                await session.scalars(
                    select(Case.failure_reason).where(Case.id == case.id)  # noqa: F821
                )
            ).all()
        )
    assert reasons == [None]
    ...
    # Slack discloses the override and never claims a probe passed.
...
@pytest.mark.asyncio
async def test_duplicate_processing_creates_no_second_override_session_or_attempt(
    rem: RemediationHarness,  # noqa: F811
    blocked: Case,
    tmp_path: Any,
) -> None:
    ...
    assert len(await _overrides(rem, case.id)) == 1
    assert await _probe_rows(rem, case.id) == (0, 0)


@pytest.mark.asyncio
async def test_dashboard_detail_discloses_the_override(
    rem: RemediationHarness,  # noqa: F811
    blocked: Case,
    tmp_path: Any,
) -> None:
    ...
    body = response.text
    assert CANARY_PROBE_OVERRIDE in body
    assert CANARY_OVERRIDE_WARNING in body
...
@pytest.mark.asyncio
async def test_superseded_approval_hash_still_blocks_under_the_override(
    rem: RemediationHarness,  # noqa: F811
    blocked: Case,
    tmp_path: Any,
) -> None:
...
@pytest.mark.asyncio
async def test_unconfirmed_label_webhook_still_blocks_under_the_override(
    rem: RemediationHarness,  # noqa: F811
    blocked: Case,
    tmp_path: Any,
) -> None:
...
@pytest.mark.asyncio
async def test_repository_outside_the_allowlist_still_blocks_under_the_override(
    rem: RemediationHarness,  # noqa: F811
    blocked: Case,
    tmp_path: Any,
) -> None:
    rem.base.settings = _with_override(
...
async def test_pr_validation_remains_mandatory_under_the_override(
    rem: RemediationHarness,  # noqa: F811
    tmp_path: Any,
...
@pytest.mark.asyncio
async def test_exact_head_ci_remains_mandatory_under_the_override(
    rem: RemediationHarness,  # noqa: F811
    tmp_path: Any,
) -> None:
    ...
    attempts = await rem.attempts(case.id)
    assert len(attempts) == 1 and attempts[0].canary_probe_override is True
    snapshots = await rem.ci(case.id)
...
async def test_blocked_case_is_claimable_only_while_the_override_is_enabled(
    ...
    worker = Worker(_canary_settings(test_database_url, live_canary_allow_missing_probe=enabled))
    try:
        claimed = await worker._claim_case()
        assert (claimed is not None) is enabled
    finally:
...
@pytest.mark.asyncio
async def test_other_blocked_reasons_are_never_claimed_by_the_override(
    integration_session_factory: Any,
    test_database_url: str,
) -> None:
    async with integration_session_factory() as session:
    ...
    worker = Worker(_canary_settings(test_database_url, live_canary_allow_missing_probe=True))
    try:
...
async def test_no_change_needed_is_refused_without_base_probe_evidence(
    ...
    assert attempt is not None and attempt.canary_probe_override is True
    assert await _probe_rows(rem, case.id) == (0, 0)
