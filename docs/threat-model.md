...
| Live calls without credentials | misconfigured deployment | `Settings` fails closed in live mode when key or org id is missing, base URL is not HTTPS, poll interval `< 10 s`, or auto-approve is enabled. |
